...
public class TC006_EndToEndTest extends BaseClass {
	...
	regpage.setFirstName(randomAlphabeticString(5).toUpperCase());
	regpage.setLastName(randomAlphabeticString(5).toUpperCase());
	...
	AccountPage mc=new AccountPage(driver);
	mc.clickLogout();
	Thread.sleep(3000);
	...
	lp.setLoginPassword("test123");
	lp.clickLoginBtn();
		
	
	System.out.println("Going to My Account Page? "+ mc.validateMyAccountHeading());
	myassert.assertEquals(mc.validateMyAccountHeading(), true); //validation
	...
	ch.setfirstName(randomAlphabeticString(5).toUpperCase());
	Thread.sleep(1000);
	ch.setlastName(randomAlphabeticString(5).toUpperCase());
	Thread.sleep(1000);
